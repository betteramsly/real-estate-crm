...
begin
  ...
  values
    (v_property_sokolniki, '2-комн. в Сокольниках, 5 мин. до метро',
     ...
     v_actor, v_actor, now() - interval '15 days', now() - interval '3 days'),
    (v_property_tverskaya, 'Студия на Тверской, для аренды',
     ...
     v_actor, v_actor, now() - interval '10 days', now() - interval '1 days'),
    (v_property_house, 'Загородный дом в Барвихе',
     ...
     v_actor, v_actor, now() - interval '30 days', now() - interval '2 days'),
    (v_property_office, 'Офис 80 м² в БЦ "Красная Пресня"',
     ...
     v_actor, v_actor, now() - interval '50 days', now() - interval '5 days'),
    (v_property_studio, 'Уютная студия рядом с метро',
     ...
     v_actor, v_actor, now() - interval '5 days', now() - interval '5 days');
